...
Revision ID: 283357f19e40
Revises: 
Create Date: 2022-11-13 10:24:12.702886
...
def upgrade():
    ...
    op.create_geospatial_table(
        ...
        sa.Column(
            'geometry', Geometry(srid=4326, spatial_index=False, from_text='ST_GeomFromEWKT', name='geometry'),
            nullable=False
        ),
        sa.Column('country_names', postgresql.JSONB(astext_type=sa.Text()), nullable=False),
        sa.Column(
            'label_point', Geometry(
    ...
    op.create_geospatial_index(
        'idx_countries_geometry', 'countries', ['geometry'], unique=False, postgresql_using='gist', postgresql_ops={}
    )
    op.create_table(
        'metadata',
    ...
    op.create_geospatial_index(
        'idx_osm_nodes_geometry', 'osm_nodes', ['geometry'], unique=False, postgresql_using='gist', postgresql_ops={}
    )
...
def downgrade():
    ...
    op.drop_geospatial_index(
        'idx_osm_nodes_geometry', table_name='osm_nodes', postgresql_using='gist', column_name='geometry'
    )
    ...
    op.drop_geospatial_index(
        'idx_countries_geometry', table_name='countries', postgresql_using='gist', column_name='geometry'
    )
